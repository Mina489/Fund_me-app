...
async function list(){
  ...
  await auctionContract.list(nftAddress,id,minprice,duration).then(() => alert('success')).catch((error) => {
...
async function end(){
  ...
  await auctionContract.end(id).then(() => alert('success')).catch((error) => {
    if(error.data) alert(error.data.message)
    else alert(error)
  })
}
...
async function bid(){
  ...
  await auctionContract.bid(id,{value:amount}).then(() => alert('success')).catch((error) => {
    if(error.data) alert(error.data.message)
    else alert(error)
  })
}
...
async function Approve(){
  ...
  await nftContract.Approve(auctionAddress,id).then(() => alert('success')).catch((error) => {
    if(error.data) alert(error.data.message)
    else alert(error)
  })
}
...
async function withdraw(){
  await getAccess()
  await auctionContract.withdrawFunds().then(() => alert('success')).catch((error) => {
    if(error.data) alert(error.data.message)
    else alert(error)
  })
}
